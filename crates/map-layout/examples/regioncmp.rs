...
fn main() {
    ...
    for r in map.rooms() {
        ...
        if let Some(m) = meta_region(r) {
            *by_meta.entry(m).or_default() += 1;
        }
    }
    println!(
        "rooms {}  with meta:region {}  with location {}  gone {}",
        map.rooms().len(),
        has_meta,
        has_loc,
        gone
    ...
    for a in &areas {
        *by_derived
            .entry(a.region.clone().unwrap_or_else(|| "(none)".into()))
            .or_default() += a.rooms.len();
    }
    ...
    for a in &areas {
        if a.region.as_deref() == Some("Wehnimer's Landing") || a.name.contains("Wehnimer") {
            ...
            println!(
                "  derived area {:?}: {} rooms, {} locations, {} GONE",
                a.name,
                a.rooms.len(),
                locs.len(),
                g
    ...
    let n: usize = map
        .rooms()
        .iter()
        .filter(|r| meta_region(r).as_deref() == Some("Wehnimer's Landing"))
        .count();
