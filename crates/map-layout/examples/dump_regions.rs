...
#
#     Five of those six are place-then-container, so the direction is
#     usually small-to-large -- but `Teras / Kharam Dzu` is the other way
#     round and the Transport ones are not containment at all, which is
#     enough to make a split unsafe.
#
#     `Zul Logoth / Kharag 'doth Dzulthu` fits the majority shape. The
#     region's 1,280 rooms are the town (584) plus its cave system --
#     The Hive, Stormpeak, Czeroth Caverns, Zaerthu Tunnels, Mraent
#     Caverns, Troll Burial Grounds -- so the second name is the
#     container. `Kharag 'doth Dzulthu` appears nowhere in gs.map, only
#     in this field.
#
